"""Command-line configuration for Stage Four PPO training."""
...
def parse_arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train TensorFlow PPO on Stage Four retreat and survival."
    )
    ...
    actor.add_argument(
        ...
        default=-1.0,
        help=(
            "Initial logit bias for RETREAT and MOVE_TO_SAFE_AREA when "
            "transferring the Stage Three actor."
        ),
    )
    ...
    initialization.add_argument(
        "--resume",
        type=Path,
        help="Stage Four actor/critic checkpoint to restore.",
    )
    initialization.add_argument(
        "--stage3-checkpoint",
        type=Path,
        help=(
            "Initialize the expanded Stage Four actor from Stage Three; "
            "the Stage Four critic starts fresh."
        ),
    )
    initialization.add_argument(
        "--from-scratch",
        action="store_true",
        help="Deliberately initialize both Stage Four networks from scratch.",
    )
