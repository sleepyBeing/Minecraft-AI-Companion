"""Command-line entry point for TensorFlow PPO Stage Four training."""
...
from agent.training.stage_4.stage_4_config import (
    PROJECT_ROOT,
    parse_arguments,
    validate_arguments,
)
from agent.training.stage_4.stage_4_models import initialize_models
from agent.training.stage_4.stage_4_reporting import (
    print_rollout,
    write_configuration,
    write_rollout_summaries,
)
from agent.training.stage_4.stage_4_rollout import (
    StageFourEpisodeCsvLogger,
    StageFourEpisodeState,
    collect_stage_four_rollout,
)
from agent.training.stage_4.stage_4_runtime import create_environment
...
def main() -> None:
    ...
    run_name = args.run_name or datetime.now().strftime(
        "stage4_retreat_survival_%Y%m%d_%H%M%S"
    )
    model_directory = PROJECT_ROOT / "agent" / "models" / "stage_4" / run_name
    log_directory = PROJECT_ROOT / "agent" / "logs" / "stage_4" / run_name
    ...
    if observation_shape is None or len(observation_shape) != 1:
        raise ValueError("Stage Four requires a one-dimensional observation space")
    observation_size = int(observation_shape[0])
    action_count = int(environment.action_space.n)
    if observation_size != 18 or action_count != 10:
        raise ValueError(
            "Stage Four requires exactly 18 observations and 10 actions"
        )
    ...
    episode_log = StageFourEpisodeCsvLogger(log_directory / "episodes.csv")
    observation, reset_info = environment.reset(seed=args.seed)
    episode_state = StageFourEpisodeState(
        starting_health=float(reset_info["starting_bot_health"])
    )
    ...
    try:
        while int(global_step.numpy()) < args.timesteps:
            ...
            rollout, observation, episode_state = collect_stage_four_rollout(
                tf=tf,
                actor=actor,
                critic=critic,
