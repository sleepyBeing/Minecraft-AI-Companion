"""Command-line entry point for TensorFlow PPO Stage Five training."""
...
from agent.training.stage_5.stage_5_config import (
    PROJECT_ROOT,
    parse_arguments,
    validate_arguments,
)
...
from agent.training.stage_5.stage_5_models import initialize_models
from agent.training.stage_5.stage_5_reporting import (
    print_rollout,
    write_configuration,
    write_rollout_summaries,
)
from agent.training.stage_5.stage_5_rollout import collect_stage_five_rollout
from agent.training.stage_5.stage_5_runtime import create_environment
...
def main() -> None:
    ...
    model_directory = PROJECT_ROOT / "agent" / "models" / "stage_5" / run_name
    log_directory = PROJECT_ROOT / "agent" / "logs" / "stage_5" / run_name
    model_directory.mkdir(parents=True, exist_ok=False)
    log_directory.mkdir(parents=True, exist_ok=False)

    observation_shape = environment.observation_space.shape
    if observation_shape != (22,) or environment.action_space.n != 12:
        raise ValueError(
            "Stage Five requires exactly 22 observations and 12 actions"
        )
    ...
    episode_log = StageFiveEpisodeCsvLogger(log_directory / "episodes.csv")
    ...
    try:
        while int(global_step.numpy()) < args.timesteps:
            ...
            rollout, observation, episode_state = collect_stage_five_rollout(
                tf=tf,
                actor=actor,
                critic=critic,
