"""Command-line configuration for Stage Five PPO training."""
...
def parse_arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Train TensorFlow PPO on Stage Five retreat, eat, and "
            "re-engagement."
        )
    )
    parser.add_argument(
    ...
    actor.add_argument(
        ...
        type=float,
        default=-1.0,
        help="Initial logit bias for EAT and REENGAGE during Stage Four transfer.",
    )
    ...
    initialization.add_argument(
        "--resume",
        type=Path,
        help="Stage Five actor/critic checkpoint to restore.",
    )
    initialization.add_argument(
        "--stage4-checkpoint",
        type=Path,
        help=(
            "Initialize the expanded actor from Stage Four; the Stage Five "
            "critic starts fresh."
        ),
    )
    initialization.add_argument(
        "--from-scratch",
        action="store_true",
        help="Initialize both Stage Five networks from scratch.",
    )
