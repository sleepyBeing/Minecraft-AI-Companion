"""Command-line entry point for TensorFlow PPO Stage Six training."""
...
from agent.training.stage_6.stage_6_config import (
    PROJECT_ROOT,
    parse_arguments,
    validate_arguments,
)
...
from agent.training.stage_6.stage_6_models import initialize_models
from agent.training.stage_6.stage_6_reporting import (
    print_rollout,
    write_configuration,
    write_rollout_summaries,
)
from agent.training.stage_6.stage_6_rollout import collect_stage_six_rollout
from agent.training.stage_6.stage_6_runtime import create_environment
...
def main() -> None:
    ...
    run_name = args.run_name or datetime.now().strftime(
        "stage6_protection_%Y%m%d_%H%M%S"
    )
    model_directory = PROJECT_ROOT / "agent" / "models" / "stage_6" / run_name
    log_directory = PROJECT_ROOT / "agent" / "logs" / "stage_6" / run_name
    model_directory.mkdir(parents=True, exist_ok=False)
    log_directory.mkdir(parents=True, exist_ok=False)

    observation_shape = environment.observation_space.shape
    if observation_shape != (30,) or environment.action_space.n != 12:
        raise ValueError(
            "Stage Six requires exactly 30 observations and 12 actions"
        )
    ...
    episode_log = StageSixEpisodeCsvLogger(log_directory / "episodes.csv")
    ...
    try:
        while int(global_step.numpy()) < args.timesteps:
            ...
            rollout, observation, episode_state = collect_stage_six_rollout(
                tf=tf,
                actor=actor,
                critic=critic,
